import { IconGolf, IconHome } from '@tabler/icons-react';
import Link from 'next/link';

export default function NotFound() {
  return (
    <div className="aurora-bg flex min-h-screen items-center justify-center p-4">
      <div className="glass-card w-full max-w-md rounded-2xl p-10 text-center animate-fade-in-up">
        <div className="mb-4 inline-flex rounded-xl bg-golf-green/10 p-4">
          <IconGolf className="h-8 w-8 text-golf-green opacity-60" aria-hidden="true" />
        </div>
        <p className="text-6xl font-extrabold text-glow">404</p>
        <h1 className="mt-3 text-xl font-bold text-white">Lost on the course?</h1>
        <p className="mt-2 text-sm text-white/50">
          This hole does not exist. Let&apos;s get you back to the clubhouse.
        </p>
        <Link
          href="/dashboard"
          className="btn-primary mt-6 inline-flex items-center gap-2 rounded-xl px-6 py-2.5 text-sm"
        >
          <IconHome className="h-4 w-4" aria-hidden="true" />
          Back to dashboard
        </Link>
      </div>
    </div>
  );
}
